...
def dcm_dir_2_niix(dcm_path, out_nii, copy_json = True):
    """
    ...
    cmd = 'dcm2niix -z y -5 -m y -f tmp/{out} "{path}"/*'.format(path = dcm_path, out= ex(out_nii).file_name)
    C = CommandRunner()
    C+=[cmd,""]
    C.run()

    tmp_files = os.listdir(os.path.join(dcm_path, "tmp"))
    nii_files = list(filter(lambda x : str(x).endswith(".nii") or str(x).endswith(".nii.gz"),tmp_files))

    if len(nii_files)<1:
        return False

    for f in tmp_files:
        _f = os.path.join(dcm_path, "tmp",f)
        if f.endswith("json"):
            if copy_json:
                py_copy(_f,os.path.join(os.path.dirname(out_nii),f))
        else:
            py_copy(_f, os.path.join(os.path.dirname(out_nii), f))
