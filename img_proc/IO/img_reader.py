...
class NibImg(object):
    ...
    def TR(self,tr):
        ...
        _step_sizes = list(self.step_sizes)
        _step_sizes[3] = tr
        self.step_sizes = tuple(_step_sizes)
        self.img.header.set_zooms(self.step_sizes)
    ...
    def c(self):
        ...
        for k,v in self.__dict__.items():
            try:
                I.__dict__[k] = copy.deepcopy(v)
            except:
                I.__dict__[k] = copy.copy(v)
    ...
    def save(self,save_path,data = None,affine = None,header = None, dtype = "float32", gz = False, RAS = False):
        ...
        H = header if not isinstance(header,NoneType) else self.img.header
        ...
        save_image(str(save_path),D,A,H,dtype = dtype, RAS  = RAS)
    ...
    def smooth(self,fwhm, inplace = True):
        ...
        if not inplace:
            return NibImg(nib.Nifti1Image(self.data, self.affine))
        else:
            return self
        ...
        if isinstance(keep_n_largest, type(None)) or (not keep_n_largest):
            for i in range(1, len(counts)):
                if counts[i][1]>min_size:
                    labeled_data[_labeled_data == counts[i][0]] = int(i)
                else:
                    break
    ...
    def dilate(self,N = 1, kernel = None, inplace = True):
        if N > 0:
            data = ndimage.binary_dilation(self.data, structure=kernel, iterations=N).astype(self.data.dtype)
        else:
            data = self.data
        if inplace:
            self.data=data
    ...
    def erode(self,N=1,kernel = None, inplace = True):
        if N > 0:
            data = ndimage.binary_erosion(self.data, structure=kernel, iterations=N).astype(self.data.dtype)
        else:
            data = self.data
        if inplace:
            self.data=data
    ...
    def close(self,N = 1,kernel = None, inplace = True):
        if N > 0:
            data = ndimage.binary_closing(self.data, structure=kernel, iterations=N).astype(self.data.dtype)
        else:
            data = self.data
        if inplace:
            self.data=data
    ...
    def open(self,N = 1,kernel = None, inplace = True):
        if N > 0:
            data = ndimage.binary_opening(self.data, structure=kernel, iterations=N).astype(self.data.dtype)
        else:
            data = self.data
        if inplace:
            self.data=data
    ...
    def morph_clean_with_recon(self, kernel = None, erosion_count = 1,inplace = True):
        ...
        seed = ndimage.binary_erosion(self.data, structure=kernel, iterations=erosion_count).astype(self.data.dtype)
    ...
    def fill_holes(self, kernel = None, inplace = True):
        data = ndimage.binary_fill_holes(self.data, structure= kernel).astype(self.data.dtype)
        if inplace:
            self.data = data
            return self
        return NibImg(nib.Nifti1Image(data, self.affine))
    ...
    def contour(self,offset=1, inset=0,contour_val = 1,kernel = None, inplace=False):
        ...
        if offset > 0:
            dilated = ndimage.binary_dilation(dilated, structure=kernel, iterations=offset)

        eroded = copy.copy(self.data)
        if inset > 0:
            eroded = ndimage.binary_erosion(eroded, structure=kernel, iterations=inset)
    ...
    def box_crop_world(self,p1w,p2w, target_origin=None, inplace = True):
        ...
        if isinstance(target_origin,list) or isinstance(target_origin,tuple):
            p1w_c = self.voxel_2_world(p1, ignore_negative_step=False)
    ...
    def box_crop_voxel(self,p1,p2, inplace = True):
        p1w = self.voxel_2_world(p1, ignore_negative_step=False)
