...
def compute_hessian_matrix(nd_array, sigma=1, scale=True):
    ...
    # smooth the nd_array
    smoothed = ndi.gaussian_filter(nd_array, sigma=sigma, mode="constant", cval=0)
    ...
    if isinstance(sigma,np.ndarray):
        #anisotorpic grid, anisotropic smoothing
        combinations = [(i,j) for i,j in combinations_with_replacement(range(ndim),2)]
        _i = range(len(combinations))
        hessian_elements = [hessian_elements[i]*sigma[ax0]*sigma[ax1] for i,(ax0,ax1) in zip(_i,combinations)]
